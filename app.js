 ...
 * @returns {Array<Object>}
 */
function computeGapStats(filtered) {
    ...
    // Group gap values by year-month of the later event
    ...
    for (const [, { year, month, gaps }] of [...byMonth.entries()].sort((a, b) => a[0] < b[0] ? -1 : 1)) {
        ...
        // Mode: round to 1 decimal for continuous hour values
        const freq = new Map();
        for (const v of gaps) {
...
function renderGapTable(filtered) {
    ...
    // Compute all gaps and per-year gap lists in one pass
